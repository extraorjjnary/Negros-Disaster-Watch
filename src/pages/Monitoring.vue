...
const filtered = computed(() => {
  if (filter.value === 'All') return sensor.stations;
  return sensor.stations.filter((s) => s.status === filter.value);
});
...
<template>
  <div class="mb-5">
    ...
    <p class="text-[13px] text-muted">
      42 IoT nodes · Auto-refresh every 5 seconds · All times in PST (UTC+8)
    </p>
  </div>
  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
    ...
    >
      ...
      <p class="text-[11px] text-muted mt-1">{{ lbl }} Sensors</p>
    </div>
  </div>
  ...
  <div class="dash-card">
    ...
    <div class="overflow-x-auto">
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="s in sensor.stations" :key="s.id" class="group">
            ...
            <td class="font-mono-custom text-[11px] text-faint">{{ now() }}</td>
          </tr>
        </tbody>
      </table>
    </div>
